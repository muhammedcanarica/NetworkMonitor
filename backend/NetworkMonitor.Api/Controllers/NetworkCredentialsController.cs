...
public sealed class NetworkCredentialsController(INetworkCredentialService service) : ControllerBase
{
    ...
    [HttpDelete("{id:int}")] public async Task<IActionResult> Delete(int id, CancellationToken token) { try { await service.DeleteAsync(id, token); return NoContent(); } catch (KeyNotFoundException) { return NotFound(); } catch (NetworkCredentialInUseException ex) { return Conflict(new ProblemDetails { Status = 409, Title = "Credential is in use", Detail = ex.Message }); } }
