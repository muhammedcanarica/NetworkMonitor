...
export function IncidentsPage() {
  ...
  return (
    <div className="page">
      <header className="page-header"><div><span className="eyebrow">Monitoring history</span><h1>Incidents</h1><p>Persistent incidents from confirmed device and interface monitoring transitions.</p></div></header>
      ...
      <section className="panel">
        ...
        {isLoading && !incidents ? <StatePanel type="loading" title="Loading incidents" message="Reading persistent incident history…" />
          : error && !incidents ? <StatePanel type="error" title="Incidents unavailable" message={error} action={<button className="button secondary" type="button" onClick={refresh}>Try again</button>} />
            : visible.length === 0 ? <StatePanel type="empty" title={filter === 'Open' ? 'No active incidents' : 'No incidents recorded'} message={filter === 'Open' ? 'All monitored devices and interfaces are currently clear.' : 'Confirmed monitoring incidents will appear here.'} />
              : <div className="table-scroll"><table className="data-table incidents-table"><thead><tr><th>Device</th><th>Status</th><th>Type</th><th>Interface</th><th>Started</th><th>Resolved</th><th>Duration</th></tr></thead><tbody>{visible.map((incident) => <IncidentRow key={incident.id} incident={incident} />)}</tbody></table></div>}
      </section>
    </div>
  )
}
...
function IncidentRow({ incident }: { incident: Incident }) {
  return <tr className={incident.status === 'Open' ? 'incident-open-row' : ''}>
    ...
    <td><strong>{formatIncidentType(incident.type)}</strong><small>{incident.summary}</small></td>
    <td>{incident.interfaceName ? <><strong>{incident.interfaceName}</strong><small>ifIndex {incident.interfaceIndex}</small></> : '—'}</td>
    <td>{formatLocalDateTime(incident.startedAt)}</td><td>{incident.resolvedAt ? formatLocalDateTime(incident.resolvedAt) : '—'}</td><td className="mono">{formatDuration(incident.durationSeconds)}</td>
  </tr>
}
